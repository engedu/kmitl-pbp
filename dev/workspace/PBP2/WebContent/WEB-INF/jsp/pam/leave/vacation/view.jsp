...
<div class="post">
	<h2 class="title">ยื่นใบลา > ใบลาพักผ่อน&nbsp;<spring:message code="${leave.cancelMsg}"/></h2>
	<div class="entry">
		...
		<table class="researchTbl">
			<tr>
				...
				<td class="text">${leave.vacationLeave.docNo}</td>
			</tr>
			
			<tr>
				<td class="label-form" width="25%">ตั้งแต่วันที่ :</td>
				<td class="text">${leave.vacationLeave.fromDateStr}</td>
			</tr>
			<tr>
				<td class="label-form" width="25%">ถึงวันที่ :</td>
				<td class="text">${leave.vacationLeave.toDateStr}</td>
			</tr>
			
			<tr>
				<td class="label-form" width="25%">มีกำหนด (วัน) :</td>
				<td class="text">
					${leave.vacationLeave.amountDay}
				</td>
			</tr>
			<tr>
				<td class="label-form" width="25%">มีวันลาพักผ่อนสะสม (วัน) :</td>
				<td class="text">
					${leave.leaveBalance}
				</td>
			</tr>
			<tr>
				<td class="label-form" width="25%">ลาพักผ่อน ในประเทศ หรือ ต่างประเทศ :</td>
				<td class="text">
					<c:if test="${leave.vacationLeave.foreign==1}">
						ต่างประเทศ  ณ ประเทศ ${leave.vacationLeave.foreignAt}
					</c:if>
					<c:if test="${leave.vacationLeave.foreign==0}">
						ในประเทศ
					</c:if>
				</td>
			</tr>
			<tr>
				...
				<td class="text">${leave.vacationLeave.contactBy}</td>
			</tr>
			<tr>
		...
		<table>
			...
			<tr>
				...
				<c:if test="${leave.vacationLeave.flagExpired == 0 }">
...
<form:hidden path="officerApprove" />
 ...
 <script type="text/javascript">
	...
	function print (){
		var form = document.forms['mainForm']; 
		form.action ="<%=request.getContextPath()%>/leave/vacation/report.htm";
		form.method='POST';	
		form.submit();
	}
	function save (){
