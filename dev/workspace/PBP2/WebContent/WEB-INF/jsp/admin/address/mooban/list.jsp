...
<form:form modelAttribute="mooban"	action="search.htm" method="POST" name="mainForm"> 	 
	  
	<div class="searchFormNoBorder"> 
	 <div class="subTopicHeaderNoBorder">Mooban &nbsp;</div> 
		<table width="100%">	
			<tr>	
				<td class="formLabel">
					Code:
    ...
    <c:if test="${empty pagingBean.currentPageItem && doSearch == 'true' }">  	 
        ...
        <c:forEach items = "${pagingBean.currentPageItem}" var="domain">
         <tr class="rgRow">     	
			 ...
			 <td align="center">    
	            <a href="<%=request.getContextPath()%>/admin/address/mooban/edit.htm?moobanId=<c:out value="${domain.moobanId}"/>" >	           
	          	<img src="<c:url value="/images/edit.png"/>" />
	            </a>  			 
			 </td>
			 <td align="center">	                  
	              <a  href="<%=request.getContextPath()%>/admin/address/mooban/delete.htm?moobanId=<c:out  value="${domain.moobanId}"/>"  onclick="return confirmPage('ยืนยันการลบข้อมูล <c:out value="${domain.name}"/> ?')">
...
<script type="text/javascript">
	function init (){
		...
		form.action ="<%=request.getContextPath()%>/admin/address/mooban/init.htm";
		form.method='GET';	
		form.submit();	
	}
	...
	function create (){		
		var form = document.forms['mainForm']; 
		form.action ="<%=request.getContextPath()%>/admin/address/mooban/create.htm";
		form.method='GET';	
		form.submit();
	}
	function chkConfirm() {
