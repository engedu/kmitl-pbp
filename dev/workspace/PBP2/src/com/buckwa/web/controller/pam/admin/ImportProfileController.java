...
@Controller
@RequestMapping("/admin/importdata/profile")
...
public class ImportProfileController {	
	...
	private static Logger logger = Logger.getLogger(ImportProfileController.class);	 	
				...
				if (originalfile.getSize() > pathUtil.getMaximumImageUploadSize()) {
					...
					while(isnext){
						...
						case 5 :
							...
							importProfileService.readPBPProfileWorkbook (workbook, httpRequest);
							step++; 
						default:
							isnext = false;
						}
	...
	public ModelAndView delete(@RequestParam("fileCode") String fileCode ,HttpServletRequest httpRequest,
		...
		try{
			...
			// Search Again
			return initProfileHistory(null ,BuckWaConstants.PROCESS_DELETE);
	...
	public ModelAndView search(@RequestParam("fileName") String search, HttpServletRequest httpRequest) {
		...
		return initProfileHistory(search, BuckWaConstants.PROCESS_SEARCH);
	}
