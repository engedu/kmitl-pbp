...
import com.buckwa.service.intf.pam.MonkhoodLeaveService;
 ...
 */
@Service("monkhoodLeaveService")
public class MonkhoodLeaveServiceImpl implements MonkhoodLeaveService {

	private static Logger logger = Logger.getLogger(MonkhoodLeaveServiceImpl.class);

	@Autowired
	private MonkhoodLeaveDao monkhoodLeaveDao;
	...
	public BuckWaResponse create(BuckWaRequest request) {
		logger.info("MonkhoodLeaveServiceImpl.flowProcess");
		BuckWaResponse response = new BuckWaResponse();
		try {
			MonkhoodLeave obj = (MonkhoodLeave) request.get("monkhoodLeave");
			
			//create leave Research
			obj = monkhoodLeaveDao.create(obj);
			...
			if(obj.getLeaveComment()!=null&&StringUtils.isNotEmpty(obj.getLeaveComment().getComment().trim())){
				...
				leaveComment.setLeaveTypeCode(BuckWaConstants.LEAVE_MONKHOOD);
			...
			leave.setLeaveTypeCode(BuckWaConstants.LEAVE_MONKHOOD);
	...
	public BuckWaResponse getById(BuckWaRequest request) {
		logger.info("MonkhoodLeaveServiceImpl.getById");
		BuckWaResponse response = new BuckWaResponse();
		try{					
			String monkhoodLeaveId =  request.get("monkhoodLeaveId")+"";			
			MonkhoodLeave obj = monkhoodLeaveDao.getById(monkhoodLeaveId);						
			if(obj!=null){
				response.addResponse("leaveMonkhood",obj);
				response.setSuccessCode("S002");
			}
		}catch(Exception ex){
			ex.printStackTrace();
	...
	public BuckWaResponse getByDocNo(BuckWaRequest request) {
		logger.info("MonkhoodLeaveServiceImpl.getByDocNo");
		BuckWaResponse response = new BuckWaResponse();
		try{					
			String docNo = request.get("docno")+"";			
			MonkhoodLeave obj = monkhoodLeaveDao.getByDocNo(docNo);						
			if(obj!=null){
				response.addResponse("monkhoodLeave",obj);
				response.setSuccessCode("S002");
			}
		}catch(Exception ex){
			ex.printStackTrace();
