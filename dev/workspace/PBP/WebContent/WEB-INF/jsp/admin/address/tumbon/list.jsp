...
<form:form modelAttribute="tumbon"	action="search.htm" method="POST" name="mainForm"> 	 
	  
	<div class="searchFormNoBorder"> 
	 <div class="subTopicHeaderNoBorder">Tumbon &nbsp;</div> 
		<table width="100%">	
			<tr>	
				<td class="formLabel">
					Code:
    ...
    <c:if test="${empty pagingBean.currentPageItem && doSearch == 'true' }">  	 
        ...
        <c:forEach items = "${pagingBean.currentPageItem}" var="domain">
         <tr class="rgRow">     	
			 ...
			 <td align="center">    
	            <a href="<%=request.getContextPath()%>/admin/address/tumbon/edit.htm?tumbonId=<c:out value="${domain.tumbonId}"/>" >	           
	          	<img src="<c:url value="/images/edit.png"/>" />
	            </a>  			 
			 </td>
			 <td align="center">	                  
	              <a  href="<%=request.getContextPath()%>/admin/address/tumbon/delete.htm?tumbonId=<c:out  value="${domain.tumbonId}"/>"  onclick="return confirmPage('ยืนยันการลบข้อมูล <c:out value="${domain.name}"/> ?')">
...
<script type="text/javascript">
	...
	function create (){		
		var form = document.forms['mainForm']; 
		form.action ="<%=request.getContextPath()%>/admin/address/tumbon/create.htm";
		form.method='GET';	
		form.submit();
	}
	function chkConfirm() {
