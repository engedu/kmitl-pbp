	...
	<div id="kpi-tree">
			<div class="searchFormNoBorder">
				<table width="100%">
					...
					<a href="#"	onclick="createNewMapping();" >
					<input value="Create" type="button" class="btn btn-primary"> 
					</a>				
					</td>			 
					</tr>
				</table>
						...
						<c:forEach items="${pagingBean.currentPageItem}" var="domain">
							<tr class="row">
								...
								<td align="left"><c:out value="${domain.categoryName}"/>&nbsp;</td>
 								<td align="left"><c:out value="${domain.status}"/>&nbsp;</td>
 								<td align="left">
 								
 								<a href="<%=request.getContextPath()%>/admin/kpiyear/getTemplateByYearAndGroupId.htm?yearId=<c:out value="${domain.yearId}"/>&groupId=<c:out value="${domain.categoryId}"/>">
								View </a>
...
<script type="text/javascript">
	...
	function createNewMapping(){		
		var form = document.forms['mainForm']; 		 	
		form.action ="<%=request.getContextPath()%>/admin/kpiyear/initCreateNewMapping.htm";	 
		form.method='GET';	
		form.submit();	
	}
