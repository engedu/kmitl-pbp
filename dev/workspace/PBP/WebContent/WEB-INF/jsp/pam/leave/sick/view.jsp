...
<div class="post">
	<h2 class="title">ยื่นใบลา > ใบลาป่วย&nbsp;<spring:message code="${leave.cancelMsg}"/></h2>
	<div class="entry">
		...
		<table class="researchTbl">
			<tr>
				...
				<td class="text">${leave.sickLeave.docNo}</td>
			</tr>
			
			<tr>
				<td class="label-form"  width="25%">ตั้งแต่วันที่ :</td>
				<td class="text">${leave.sickLeave.fromDateStr}</td>
			</tr>
			<tr>
				<td class="label-form"  width="25%">ถึงวันที่ :</td>
				<td class="text">${leave.sickLeave.toDateStr}</td>
			</tr>
			
			<tr>
				<td class="label-form"  width="25%">มีกำหนด (วัน) :</td>
				<td class="text">
					${leave.sickLeave.amountDay}
				</td>
			</tr>
			<tr>
				<td class="label-form"  width="25%">มีวันลาป่วยคงเหลือ (วัน) :</td>
				<td class="text">
					${leave.leaveBalance}
				</td>
			</tr>
			<tr>
				...
				<td class="text">${leave.sickLeave.reason}</td>
			</tr>
			<tr>
				<td class="label-form"  width="25%">ในระหว่างลาจะติดต่อข้าพเจ้าได้ที่ :</td>
				<td class="text">${leave.sickLeave.contactBy}</td>
			</tr>
			<tr>
		...
		<table>
			...
			<tr>
				...
				<c:if test="${leave.sickLeave.flagExpired == 0 }">
...
<form:hidden path="officerApprove" />
 ...
 <script type="text/javascript">
	...
	function print (){
		var form = document.forms['mainForm']; 
		form.action ="<%=request.getContextPath()%>/leave/sick/print.htm";
		form.method='GET';	
		form.submit();	
	}
