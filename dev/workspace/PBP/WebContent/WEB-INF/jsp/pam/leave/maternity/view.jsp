...
<form:form modelAttribute="leave" action="view.htm" method="POST"
	name="mainForm">

	<div class="post">
	<h2 class="title">ยื่นใบลา > ใบลาคลอด&nbsp;<spring:message code="${leave.cancelMsg}"/></h2>
	<div class="entry"><%@include file="/WEB-INF/jsp/pam/leave/includeProfile.jsp"%>
	<div class="line">&nbsp;</div>
	<div style="clear: both;">&nbsp;</div>
	<table class="researchTbl">
		<tr>
			<td class="label-form" width="25%">รหัสเอกสาร :</td>
			<td class="text">${leave.maternityLeave.docNo}</td>
		</tr>

		<tr>
			<td class="label-form" width="25%">ตั้งแต่วันที่ :</td>
			<td class="text">${leave.maternityLeave.fromDateStr}</td>
		</tr>
		<tr>
			<td class="label-form" width="25%">ถึงวันที่ :</td>
			<td class="text">${leave.maternityLeave.toDateStr}</td>
		</tr>

		<tr>
			<td class="label-form" width="25%">มีกำหนด (วัน) :</td>
			<td class="text">${leave.maternityLeave.amountDay}</td>
		</tr>
		<tr>
			<td class="label-form" width="25%">มีวันลาคลอดคงเหลือ (วัน) :</td>
			<td class="text">${leave.leaveBalance}</td>
		</tr>
		<tr>
			<td class="label-form" width="25%">เนื่องจาก :</td>
			<td class="text">${leave.maternityLeave.reason}</td>
		</tr>
		<tr>
			<td class="label-form" width="25%">ในระหว่างลาจะติดต่อข้าพเจ้าได้ที่
			:</td>
			<td class="text">${leave.maternityLeave.contactBy}</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><form:textarea path="leaveComment.comment"
				id="leaveComment.comment" /></td>
		</tr>
	</table>
	<div class="line">&nbsp;</div>
	...
	<table>
		...
		<tr>
			<td align="center">
				...
				<c:if test="${leave.maternityLeave.flagExpired == 0 }">
...
<script type="text/javascript">
	...
	function print (){
		var form = document.forms['mainForm']; 
		form.action ="<%=request.getContextPath()%>/leave/maternity/print.htm";
		form.method='GET';	
		form.submit();	
	}
