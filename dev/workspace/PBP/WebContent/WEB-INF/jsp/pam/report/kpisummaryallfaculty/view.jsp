...
<div class="post">
<h2 class="title"><spring:message code="label.report.kpisummary.title.allfaculty" /> (5)</h2>
<div class="entry">
<br/>
...
<span style="color: black;">
	...
	<% if (PAMConstants.PERSON_TYPE_CODE_OFFICER.equals(request.getAttribute("selectedPersonType"))) { %>
		<a href="#" onclick="callKpiAllFaculty(<%= PAMConstants.PERSON_TYPE_CODE_GOV_OFFICER %>)">
	<% } %>
	...
	<% if (PAMConstants.PERSON_TYPE_CODE_GOV_OFFICER.equals(request.getAttribute("selectedPersonType"))) { %>
		<a href="#" onclick="callKpiAllFaculty(<%= PAMConstants.PERSON_TYPE_CODE_OFFICER %>)">
	<% } %>
...
<div style="clear: both;">&nbsp;</div>
	...
	<tr>
		<td align="center">
			<input class="btn btn-primary" value="<spring:message code="label.button.excel"/> ภาพรวมของสาขาทุกสาขา" type="button" id="exportToExcelBtn" rel='notLoading'>
			<!-- <input class="btn btn-primary" value="ดูสรุปรายสาขา" type="button" id="viewByFacultyBtn" rel='notLoading'> -->
		</td>
	</tr>
	</table>
</div>
...
<script type="text/javascript">
	...
	$(document).ready( function() {
		...
		$("#exportToExcelBtn").click(function(){
			...
			form.action = "<%=request.getContextPath()%>/pam/report/kpisummaryallfaculty/exportToExcel.htm";
			form.method = 'POST';
			form.submit();
		});
	
		$("#viewByFacultyBtn").click(function(){
			var form = document.forms['mainForm'];
			form.action = "<%=request.getContextPath()%>/pam/report/kpisummaryallfaculty/initByFaculty.htm";
			form.method = 'POST';
			form.submit();
		});
	...
	function callKpiAllFaculty(selectedPersonType){
		var form = document.forms['mainForm'];
		form.action = "<%=request.getContextPath()%>/pam/report/kpisummaryallfaculty/init.htm";
		form.method = 'GET';
	...
	function callKpiByTopic(kpiId, personType){
		var form = document.forms['mainForm'];
		form.action = "<%=request.getContextPath()%>/pam/report/kpisummaryallfaculty/initByTopic.htm";
		form.method = 'POST';
