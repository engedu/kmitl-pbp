...
<form:form modelAttribute="workPerson" action="create.htm" method="POST" name="mainForm" enctype="multipart/form-data">
<div class="post">
	<h2 class="title">ผลงาน > สร้าง</h2>
	<div class="entry">
		<div style="clear: both;">&nbsp;</div>
				...
				<c:forEach items="${workPerson.workPersonAttrList}" var="domain" varStatus="row"> 
					<tr>
						...
						<td>
							...
							<form:hidden path="workPersonAttrList[${row.index}].kpiId"/>
							<form:hidden path="workPersonAttrList[${row.index}].isFile"/>
						</td>
					</tr>
				</c:forEach>
			</table>
			<c:if test="${not empty workPerson.workTemplateAttrMappingList}">
			<div class="line">&nbsp;</div>
			<div style="clear: both;">&nbsp;</div>
			<div>
				<table width="100%">
					...
					<tr>
						...
						<td class="text">
							<form:select path="workPersonAttrMappingId" id="workPersonAttrMappingId">
								<form:options items="${workPerson.workTemplateAttrMappingList}" itemValue="workTemplateAttrId" itemLabel="label" />
							</form:select>
							<span class="require">*</span>
							<form:errors path="workPersonAttrMappingId" cssClass="require" />
						</td>
					</tr>
				</table>
			</div>
			...
			<tr>
				<th class="headerTH" width="10px">ที่</th>
				<th class="headerTH" width="120px">ชื่อไฟล์</th>
				<th class="headerTH" width="80px">ผูกกับคุณลักษณะ</th>
				<th class="headerTH" width="20px">ดาวน์โหลด</th>
				<th class="headerTH" width="20px">ลบ</th>
			</tr>
			</thead>
...
function validateDelete (fileName, fileCode){
	...
	if (flag) {
		...
		window.location ="<%=request.getContextPath()%>	/pam/work/deleteFile.htm?fileCode=" + fileCode + "&fileName=" + fileName;
	}
}
